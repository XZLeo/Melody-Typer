...
# * However, using a deeper network doesn't always help. <u>A huge barrier to training them is vanishing gradients</u>: very deep networks often have a gradient signal that goes to zero quickly, thus making gradient descent prohibitively slow. 
...
# 
# <u>We also saw in lecture that having ResNet blocks with the shortcut also makes it very easy for one of the blocks to learn an identity function. This means that you can stack on additional ResNet blocks with little risk of harming training set performance.</u>  
#     
...
# 
# The <u>identity block is the standard block</u> used in ResNets, and corresponds to the case where <u>the input activation (say $a^{[l]}$) has the same dimension as the output activation (say $a^{[l+2]}$)</u>. To flesh out the different steps of what happens in a ResNet's identity block, here is an alternative diagram showing the individual steps:
# 
...
# 
# In this exercise, you'll actually implement <u>a slightly more powerful version</u> of this identity block, in which the skip connection "skips over" 3 hidden layers rather than 2 layers. It looks like this: 
# 
...
def identity_block(X, f, filters, stage, block):
    ...
    # Retrieve Filters
    F1, F2, F3 = filters # F1是第一卷积的Nc，channel数
...
# 
# The ResNet "convolutional block" is the second block type. You can use this type of block <u>when the input and output dimensions don't match up.</u> The **difference** with the identity block is that there is a CONV2D layer in the shortcut path: 
# 
# <img src="images/convblock_kiank.png" style="width:650px;height:150px;">
# <caption><center> <u> <font color='purple'> **Figure 4** </u><font color='purple'>  : **Convolutional block** </center></caption>
# 
# * The CONV2D layer in the shortcut path is used to<u> resize the input $x$ to a different dimension</u>, so that the dimensions match up in the final addition needed to add the shortcut value back to the main path. (This plays a similar role as the matrix $W_s$ discussed in lecture.) 
# * For example, to reduce the activation dimensions's height and width by a factor of 2, you can use a 1x1 convolution with a stride of 2. 
# * The CONV2D layer on the shortcut path does <u>not use any non-linear activation function</u>. Its main role is to just apply a (learned) linear function that reduces the dimension of the input, so that the dimensions match up for the later addition step. 那矩阵的参数怎么选？
# 
